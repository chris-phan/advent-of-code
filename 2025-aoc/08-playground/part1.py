...
if __name__ == "__main__":
    ...
    freq: dict[int, int] = {}
    for cluster in point_to_cluster.values():
        parent = get_parent_cluster(cluster)

        if freq.get(parent, -1) == -1:
            freq[parent] = 0
        freq[parent] += 1

    top_freq = sorted(freq.values(), reverse=True)
    res = top_freq[0] * top_freq[1] * top_freq[2]
    print(f"res: {res}")
